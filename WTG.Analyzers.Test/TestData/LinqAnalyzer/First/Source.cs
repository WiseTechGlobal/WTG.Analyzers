...
public class Bob
{
	public int Method(string source) => source.First();
	public int Method(int[] source) => source.First();
	public int Method(IReadOnlyList<int> source) => source.First();
	public int Method(IReadOnlyCollection<int> source) => source.First();
	public int Method(IList<int> source) => source.First();
	public int Method(ICollection<int> source) => source.First();
	public int Method(IEnumerable<int> source) => source.First();

	public int MethodA(int[] source) => Enumerable.First(source);
	public int MethodB(int[][] source) => source.First().First();
	public Expression<Func<int[], int>> Query = source => source.First();
